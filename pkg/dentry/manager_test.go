...
var _ = Describe("TestChangeEntryParent", func() {
	...
	Context("mv file src grp1 and files", func() {
		It("should be succeed", func() {
			...
			entryUri, err := entryManager.GetEntryByUri(context.TODO(), "/test_mv_grp2/test_mv_grp2_file1")
			Expect(err).Should(BeNil())
			Expect(entryUri.ID).Should(Equal(grp1File1.ID))
		})
		...
		It("has existed file should be succeed if enable replace", func() {
			...
			Expect(err).Should(BeNil())
			Expect(doc.Desync).Should(BeTrue())
			entryUri, err := entryManager.GetEntryByUri(context.TODO(), "/test_mv_grp2/test_mv_grp2_file2")
			Expect(err).Should(BeNil())
			Expect(entryUri.ID).Should(Equal(grp1File2.ID))
		})
	})
})
